...
#    """Given a filename, returns if it is available on the server
#
#    """
...
def mark_beam_downloaded(filename):
    db = database.Database(dbname)
    #TODO with a LEFT JOIN
    QUERY = "UPDATE full_processing SET status='downloaded' WHERE filename='%s'"%filename
    db.cursor.execute(QUERY)
    db.conn.close()
...
def download(attempt):
    ...
    cmd = "bbftp -e 'setoption localrfio; setoption notmpfile; get %s %s' -u %s %s -E \"/usr/local/bin/bbftpd -s\""%(file['remote_filename'], config.download.datadirbbftp, config.commondb.username, config.commondb.host)
