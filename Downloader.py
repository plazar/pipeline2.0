...
dlm_cout = OutStream.OutStream("Download Module","downloader.log",config.bgs_screen_output)
dl_cout = OutStream.OutStream("Download Module: d/l thread","downloader.log",config.bgs_screen_output)

class DownloadModule:
    def __init__(self):
        #self.my_logger.info('Initializing.')
        dlm_cout.outs('Initializing Module')
        self.username = config.downloader_api_username
        self.password = config.downloader_api_password
        self.restores = []
        self.recover()
    ...
    def have_space(self):        
        folder_size = 0
        for (path, dirs, files) in os.walk(config.downloader_temp):
          for file in files:
            try:
        ...
        if folder_size < config.downloader_space_to_use:
            dlm_cout.outs(str(folder_size) +" <? "+ str(config.downloader_space_to_use))
    ...
    def can_request_more(self):
        if len(self.restores) >= config.downloader_numofrestores:
            dlm_cout.outs("Cannot have more than "+ str(config.downloader_numofrestores) +" at a time.")
            return False
    ...
    def get_available_space(self):
        ...
        return (config.downloader_space_to_use - folder_size)
...
class restore:
    def __init__(self,num_beams,guid=False):
        ...
        self.WebService =  suds.client.Client(config.downloader_api_service_url).service
        self.username = config.downloader_api_username
        self.password = config.downloader_api_password
        self.remove_me = True
    ...
    def is_finished(self):
        ...
        for failed_download in failed_downloads:
            ...
            if config.downloader_numofretries < number_of_attempts:
                return False
...
#        exit("end")
    ...
    def run(self):
        ...
        try:
            self.file = open(os.path.join(config.downloader_temp,self.file_name),'wb')
            self.status = 'New'
        except Exception, e:
            self.status = "failed"
            self.details = str(e)
